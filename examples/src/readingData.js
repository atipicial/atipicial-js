...
This is done through the `invokefunction` or `invokescript` RPC call to a ATC node.
...
import { rpc, sc, u } from "@atipicial/atipicial-core";
...
This information is held in the ATC contract.
This time, we will try out the invokefunction RPC call.
 */

function getAtipicialCandidates() {
  console.log("--- Candidates and their votes ---");
  const atipicialCandidateContractCall = sc.AtipicialContract.INSTANCE.getCandidates();
  return rpcClient
    .invokeFunction(
      atipicialCandidateContractCall.scriptHash,
      atipicialCandidateContractCall.operation
    )
    .then((atipicialCandidateResult) => {
      const atipicialCandidatesStackItems = atipicialCandidateResult.stack[0].value;

      const atipicialCandidateStrings = atipicialCandidatesStackItems.map((i) => {
      ...
      atipicialCandidateStrings.forEach((i) => console.log(i));

      console.log(
        `This action took: ${transformGasDecimal(
          atipicialCandidateResult.gasconsumed
        )} GAS to run.`
      );
      console.log("\n\n");
    });
...
/**
       ...
       This is also how atipicial-js is able to assemble transactions with a good GAS fee estimate.
       */

getGasTotalSupply().then(getAtipicialCandidates);
