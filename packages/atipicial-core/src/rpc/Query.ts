...
export interface GetAep11BalancesResult {
...
export interface GetAep11TransfersResult {
  address: string;
  sent: Aep11TransferEvent[];
  received: Aep11TransferEvent[];
}

export interface Aep11TransferEvent {
  timestamp: number;
  assethash: string;
...
export interface GetAep17TransfersResult {
  sent: Aep17TransferEvent[];
  received: Aep17TransferEvent[];
  address: string;
}

export interface Aep17TransferEvent {
  timestamp: number;
  assethash: string;
  transferaddress: string;
  amount: string;
...
export interface GetAep17BalancesResult {
  address: string;
  balance: {
    assethash: string;
    amount: string;
...
/**
 * A Query object helps us to construct and record requests for the Atipicial node RPC. For each RPC endpoint, the equivalent static method is camelcased. Each Query object can only be used once.
 *
 * @example
...
export class Query<TParams extends JsonRpcParams, TResponse> {
  ...
  public static getAep11Balances(
    accountIdentifier: string,
  ): Query<[string], GetAep11BalancesResult> {
    return new Query({
      method: "getaep11balances",
      params: [accountIdentifier],
    });
  }

  public static getAep11Properties(
    contractHash: string,
    tokenId: string,
  ): Query<[string, string], Record<string, unknown>> {
    return new Query({
      method: "getaep11properties",
      params: [contractHash, tokenId],
    });
  }

  public static getAep11Transfers(
    accountIdentifer: string,
    startTime?: string,
    endTime?: string,
  ): Query<[string, string?, string?], GetAep11TransfersResult> {
    const params: [string, string?, string?] = [accountIdentifer];
    if (startTime) params.push(startTime);
    if (endTime) params.push(endTime);
    return new Query({
      method: "getaep17transfers",
      params,
    });
  }

  /**
   * Query returning the Aep17 transfer history of an account. Defaults of 1 week of history.
   ...
   */
  public static getAep17Transfers(
    accountIdentifer: string,
    startTime?: string,
    endTime?: string,
  ): Query<[string, string?, string?], GetAep17TransfersResult> {
    const params: [string, string?, string?] = [accountIdentifer];
    if (startTime) params.push(startTime);
    if (endTime) params.push(endTime);
    return new Query({
  ...
  /**
   * Query returning the Aep17 balance of an account.
   * @param accountIdentifer - address or scriptHash of account
   */
  public static getAep17Balances(
    accountIdentifer: string,
  ): Query<[string], GetAep17BalancesResult> {
    return new Query({
  ...
  /**
   * This Query returns the available unclaimed bonus GAS for a ATC address
   * @param addr - a ATC address
   */
