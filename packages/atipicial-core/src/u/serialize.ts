...
export interface AtipicialSerializable {
  size: number;
  serialize: () => string;
}
type Serializables = number | HexString | AtipicialSerializable[];
/**
 * Calculates the byte size of any supported input following ATC's variable int format.
 */
...
export function serializeArrayOf(prop: (AtipicialSerializable | string)[]): string {
  return (
