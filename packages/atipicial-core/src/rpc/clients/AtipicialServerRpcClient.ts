...
export function AtipicialServerRpcMixin<TBase extends RpcDispatcherMixin>(
  base: TBase,
) {
  return class AtipicialServerRpcInterface extends base {
    //#region Blockchain
    ...
    /**
     * Gets the version of the ATC node along with various other metadata.
     */
    ...
    /**
     * Get the amount of unclaimed GAS for a ATC address. This is returned as the raw value. To get the display value, divide this by 100000000.
     */
    ...
    /**
     * Checks if the provided address is a valid ATC address.
     */
...
/**
 * RPC Client model to query a ATC node. Contains built-in methods to query using RPC calls.
 */
export class AtipicialServerRpcClient extends AtipicialServerRpcMixin(RpcDispatcher) {
  public get [Symbol.toStringTag](): string {
    return `AtipicialServerRpcClient(${this.url})`;
  }
}
