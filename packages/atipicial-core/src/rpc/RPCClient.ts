import { AtipicialServerRpcMixin } from "./clients/AtipicialServerRpcClient";
import {
  RpcDispatcher,
...
class FullRpcClient extends TokenTrackerRpcMixin(
  ApplicationLogsRpcMixin(AtipicialServerRpcMixin(RpcDispatcher)),
) {
...
export class RPCClient extends FullRpcClient {
  ...
  /**
   * @param net - A url pointing to a ATC RPC node.
   * @param version - version of ATC node. Used to check if RPC methods have been implemented. it will default to DEFAULT_RPC found in CONST
   */
