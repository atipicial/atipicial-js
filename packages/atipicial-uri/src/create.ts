 ...
 *
 * ```ts
 * import {createPayUri} from "@atipicial/atipicial-uri";
...
export function createPayUri(
  ...
  let uri = `atipicial:${toAddress}?asset=${asset}`;
  if (amount) {
    uri += `&amount=${amount}`;
  }
