...
describe("JSON", () => {
  ...
  test.each(jsonTestCases)(
    "fromJson %s",
    (_: string, json: WitnessRuleJson, atipicialObj: WitnessRule) => {
      const result = WitnessRule.fromJson(json);
      expect(result.action).toEqual(atipicialObj.action);
      expect(result.condition).toEqual(atipicialObj.condition);
    },
  );
  ...
  test.each(jsonTestCases)(
    "toJson %s",
    (_: string, json: WitnessRuleJson, atipicialObj: WitnessRule) => {
      const result = atipicialObj.toJson();
