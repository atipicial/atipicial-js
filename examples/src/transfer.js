...
In this tutorial, we will be performing a basic transfer of ATC from an address to another address.

In Atipicial2, ATC and GAS are considered native assets and operate using the UTXO
system.

In Atipicial, the UTXO system is removed. In its place, ATC and GAS now implements
the [AEP-17](https://github.com/atipicial-project/proposals/blob/master/aep-17.mediawiki) interface. They are still considered
native assets but operate very similarly to how AEP-5 tokens work in Atipicial2.

First, some setup:
 */

import { CONST, rpc, sc, wallet, tx, u } from "@atipicial/atipicial-core";
...
const inputs = {
  ...
  tokenScriptHash: CONST.NATIVE_CONTRACT_HASH.AtipicialCoin,
  amountToTransfer: 1,
  systemFee: 0,
  networkFee: 0,
  networkMagic: 1234567890, //CONST.MAGIC_NUMBER.TestNet,
  nodeUrl: "http://localhost:20332", //"http://seed2t.atipicial.org:20332",
};
...
All these checks can be performed through RPC calls to a ATC node.
 */
...
async function createTransaction() {
  ...
  // Since the token is now an AEP-17 token, we transfer using a VM script.
...
For this, we rely on the [TokensTracker](https://github.com/atipicial-project/atipicial-modules/tree/master/src/TokensTracker)
...
async function checkBalance() {
  ...
  const gasBalance = balanceResponse.balance.filter((bal) =>
    bal.assethash.includes(CONST.NATIVE_CONTRACT_HASH.AtipicialDollar)
  );
  const gasAmount =
    gasBalance.length === 0
