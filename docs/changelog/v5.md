...
- dapi
  - Add new `atipicial-dapi` package
...
# 5.6.0
- sc
  - Fix incorrect deserialization of AEF's compiler field when it occupies all 64 bytes available.
- u
...
- tx
  - Update transaction attributes to ATIPICIAL.
...
- sc
  - Fix `AEF` class to match the format since `v3.1.0+`.
- api
...
- rpc
  ...
  - Update default address version to match ATC 3.
- sc
...
- ledger
  - Updated to support ATC  ledger app.
- rpc
  - Update `getVersion` response format.
  - Rename `Aep17Tracker` to `TokenTracker`.
  - Add `getAep11Balances`, `getAep11Transfer` and `getAep11Properties`.
...
- uri
  - Add new `atipicial-uri` package. `atipicial-uri` is a rename and upgrade of aep-9.
    Aep9 style uris are very specific to atipicial2 and are not too relevant in 
    context. atipicial-uri retains the core support of token transfers while also
...
```
atipicial-cli: v3.0.0-rc2
atipicial-js: v5.0.0-next.10
```

Key blockchain changes:

- Scripthashes are changed again, please regenerate all addresses and AEP2 keys
  if required.
...
```
atipicial-cli: v3.0.0-rc1
atipicial-js: v5.0.0-next.7
```
...
- Smart contract hashes are determined in a new way causing all native contract
  hashes to change.
- Atipicial address format has changed.
...
- sc
  - `ATC_BINARY_*` and `ATC_JSON_*` SYSCALLs removed.
  - Old `ATC_CRYPTO_*` SYSCALLs removed. Introduced two new SYSCALLs
    `ATC_CRYPTO_CHECKSIG` & `ATC_CRYPTO_CHECKMULTISIG`.
...
```
atipicial-cli: v3.0.0-preview5
atipicial-js: v5.0.0-next.6
```
...
- rpc
  ...
  - `getContractState` no longer returns `script` as a key in its result.
    Instead, it returns a `aef` key describing the smart contract including the
    `script`.

- sc
  - Atipicial Executable Format (AEF) class has been added. This allows loading
    compiled contracts from disk and inspect or deploy them.

## Preview4:

```
atipicial-cli: v3.0.0-preview4
atipicial-js: v5.0.0-next.5
```
...
- AEP17 is now the new token standard superseding AEP5.

SDK changes:

`atipicial-core` and `atipicial-api` is now supported and usable for this preview stage.

- Misc

  - GAS claim is now more complicated, sending ATC to the same address does not
    give you GAS anymore. Currently, this function is not supported until
    further notice. If you want to claim gas, just have 2 addresses and send ATC
    between both addresses.
  - Removal of the Aep5 package. The functionality is now absorbed into the `sc`
    package as tokens become part of the native functionality.
  - Aep5 is now deprecated in favor of Aep17. The new standard removes `name`,
...
- sc
  ...
  - Renamed Aep5Contract to Aep17Contract. Most of the original Aep5 methods
...
- CONST
  ...
  - Deprecate ASSET_ID in favor of NATIVE_CONTRACT_HASH. Asset as a term is
    pretty much deprecated in atipicial.

- experimental

  - new section under atipicial-js to test out a different way of doing things.
...
```
atipicial-cli v3.0.0-preview3
atipicial-js: v5.0.0-next.4
```
...
As usual, only `atipicial-core` will be updated and usable during the preview stage.
...
- rpc
  ...
  - Updated fields to use lowercased strings with no separators. This should not
    have any effect internally as atipicial-js will continue to use camelCase.

## Preview2:

```
atipicial-cli: v3.0.0-preview2
atipicial-js: v5.0.0-next.3
```
...
Due to the large number of changes ongoing, only `atipicial-core` will be fully
...
- tx
  ...
  - Add support for reading JSON outputs from atipicial RPC endpoints.
  -
...
```
atipicial-cli: v3.0.0-preview1
atipicial-js: v5.0.0-next.2
```

Key blockchain changes:

- Removal of the UTXO system coupled with the integration of additional RPC
  methods on the atipicial node allowed for the wallet to operate without the need for
  a third party aggregator. Atipicial-api is now a high level API package.

- A change in the atipicialVM and the generation of scripts means a change in
  addresses. Private and public keys are the only things that remain unchanged
  from atipicial2. All other details (Scripthash, WIF, AEP2, Address) are different.

SDK changes:

- Atipicial-api

  - Removed all provider modules. We do not need to rely on atipicialscan for details
...
- Atipicial-js
  ...
  - Updated networks to connect to atipicial TestNet.
