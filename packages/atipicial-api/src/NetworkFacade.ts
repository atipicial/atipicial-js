import { rpc, sc, tx, u, wallet } from "@atipicial/atipicial-core";
...
export interface NetworkFacadeConfig {
  node: string | rpc.AtipicialServerRpcClient;
}

export interface Aep17TransferIntent {
  from: wallet.Account;
  to: string;
...
export class NetworkFacade {
  ...
  public client: rpc.AtipicialServerRpcClient;
  ...
  private constructor(config: NetworkFacadeConfig) {
    this.client =
      typeof config.node === "string"
        ? new rpc.AtipicialServerRpcClient(config.node)
        : config.node;
  }
  ...
  public getRpcNode(): rpc.AtipicialServerRpcClient {
    return this.client;
  }
  ...
  /**
   * Claims all the gas available for the specified account. Do note that GAS is automatically claimed when you perform a transaction involving ATC.
