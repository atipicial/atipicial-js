...
- Fixes
  ...
  - Fix AEP5 parsing of contract return values.

- Others

  - Update crypto-js to 3.3.0
  - Update elliptic to 6.5.3
  - Minor fixes to tests to not use testnet atipicialscan as it is broken.
  - Remove default notifications urls as they are being deprecated.

# 4.8.0

This is likely the last major feature version for atipicial2. The next major version
is expected to support only atipicial given the breaking changes.
...
- API
  ...
  - Add notifications API. This is a pubsub model using websockets to listen to
    ...
    [demo](https://jsfiddle.net/gh/get/jquery/3.0/atipicial/atipicial-js/tree/master/examples/browser/subscribenotifications/demo).

- Ledger

  - `atipicial-ledger` is a helper package which wraps the functionality for
...
- Fixes
  ...
  - Fix Fixed8 overflow in aep5 transfer method
...
- Atipicial-core
  ...
  - Add parsing capabilities for RegisterTransaction and EnrollmentTransaction.
    At this point, all transaction types in atipicial 2.x are now fully supported.
  - Update axios dependency due to minor security concern.

- Docs

  - Add jsfiddle docs describing atipicial RPC.
...
- Fixes
  ...
  - Update VM status output for atipicial v2.10.2
...
- Fixes
  ...
  - Fix atipicial-api httpsOnly setting not being propagated correctly.
...
- Atipicial-api
  ...
  - Add atipicialCli API Client. Do not that not all endpoints are currently
...
- Fixes
  ...
  - Correct error message in aep2.decrypt (Wrong capitalization of second word)
...
- Fixes
  ...
  - Fix exported files for atipicial-core. Previously, the `package.json` was
...
- Atipicial-AEP9
  ...
  - Ability to parse AEP9 QR codes. Do note that this does not come with a QR
...
- Publishing
  ...
  > Version 4 will mark the start of the mono-repository for atipicial-js. This means
  > that there will be multiple packages being published from this repository.
  > `atipicial-js` will still fulfil the role of the package for browser based
  > interaction with the ATC blockchain. `atipicial-core` is the core package that
  > contains most of the code from the old `atipicial-js`. However, the semantic API
  > is moved to `atipicial-js` and will not be included in the core package.
  > `atipicial-api` and `atipicial-aep5` are extracted plugin packages that relies on the
  > `atipicial-core` package. They contain functionality that is not integral to the
  > workings of the blockchain but are useful in many situations.

- API

  > API is now extracted into its own package under `atipicial-api`. The core
  ...
  - Removed AEP5 methods from API. They now live in the `atipicial-aep5` package.
  - Removed `cmc` from API.

- AEP5

  > AEP5 is now extracted into its own package under `atipicial-aep5`. The core
...
- Core
  ...
  > `atipicial-core` forms the base package and contains all the modules required for
