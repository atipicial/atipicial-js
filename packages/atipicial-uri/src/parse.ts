import type { sc } from "@atipicial/atipicial-core";

const INTENT_PREFIXES = ["pay", "vote"];

// We hardcode these so we can avoid a full dependency.
const NATIVE_TOKENS = {
  atipicial: "ef4073a0f2b305a38ec4050e4d3d28bc40ea63f5",
  gas: "d2a4cff31913016155e38e474a2c06d08be276cf",
};

/**
 * Results of parsing a ATC uri. Contains a displayable string describing the intent and a partial ContractCall object that requires more information to be used in a Transaction.
 */
...
/**
 * Parses an AEP9 URI string into a consumable intent object. This function does not check for runtime validity conditions (eg address or contract validity).
...
export function parse(uri: string): UriIntent {
  ...
  if (!uriBody) {
    throw new Error("URI did not contain anything after atipicial:");
  }
...
function validateScheme(scheme: string): void {
  if (scheme !== "atipicial") {
    throw new Error("URI provided did not start with atipicial");
  }
}
...
function createPayDescription(
  ...
  const tokenName =
    assetString === "atipicial" || assetString === "gas"
...
function parseAssetToScriptHash(assetString: string): string {
  if (assetString === "atipicial" || assetString === "gas") {
...
function createVoteIntent(
  ...
  return {
    ...
    contractCall: {
      scriptHash: NATIVE_TOKENS.atipicial,
      operation: "vote",
      args: [
        { type: "Hash160", value: "" },
