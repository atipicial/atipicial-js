...
atipicial-js without much configuration. The primary aim of the facade is to provide
single "batteries included" methods that developers can rely on to quickly build
out an interaction.

For this tutorial, you will need:

- A url which is a ATC node JSON RPC endpoint.
...
const facadePromise = Atipicial.api.NetworkFacade.fromConfig({
  node: url,
});
```
...
```js
const intent = {
  from: new Atipicial.wallet.Account(privateKey),
  to: address,
  decimalAmt: 0.00000001,
  contractHash: Atipicial.CONST.NATIVE_CONTRACT_HASH.AtipicialDollar,
};
```
...
```js
const signingConfig = {
  signingCallback: Atipicial.api.signWithAccount(
    new Atipicial.wallet.Account(privateKey)
  ),
};
```
