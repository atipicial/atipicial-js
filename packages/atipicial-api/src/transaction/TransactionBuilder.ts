import { tx, sc, u, wallet } from "@atipicial/atipicial-core";
...
export class TransactionBuilder {
  ...
  /**
   * Adds the logic for claiming gas. Alternatively, you may just trigger the gas claim by performing an actual transaction involving ATC.
  ...
  public addGasClaim(account: wallet.Account): TransactionBuilder {
    ...
    return this.addContractCall(
      sc.AtipicialContract.INSTANCE.transfer(address, address, 0),
  ...
  public addAep17Transfer(
    ...
    const address = account.address;
    const contract = new sc.Aep17Contract(tokenScriptHash);
    return this.addContractCall(
   ...
   * @param account - Account containing the ATC.
  ...
  public addVote(
    ...
    return this.addContractCall(
      sc.AtipicialContract.INSTANCE.vote(address, candidatePublicKey),
    ).addBasicSignatureField(account);
  }
