import { u, wallet } from "@atipicial/atipicial-core";
...
jest.mock("@atipicial/atipicial-core", () => {
  const actual = jest.requireActual("@atipicial/atipicial-core");
  return {
    ...actual,
    rpc: {
      ...actual.rpc,
...
describe("DapiOperations.getStorage", () => {
  ...
  test("converts base64 key to hex before calling rpcClient", async () => {
    ...
    const { u: uActual } = jest.requireActual("@atipicial/atipicial-core");
...
describe("DapiOperations.makeTransaction", () => {
  ...
  test("returns correct type field", async () => {
    ...
    expect(result.type).toBe("Atipicial.Network.P2P.Payloads.Transaction");
  });
...
describe("DapiOperations.relay", () => {
  const context = {
    type: "Atipicial.Network.P2P.Payloads.Transaction" as const,
    hash: TXID,
    network: DapiNetwork.TESTNET,
    data: "c2lnbmVkdHg=",
    items: {},
