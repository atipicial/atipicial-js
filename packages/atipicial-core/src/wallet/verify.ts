...
/**
 * Verifies a AEP2. This merely verifies the format. It is unable to verify if it is has been tampered with.
 */
export function isAEP2(aep2: string): boolean {
  try {
    if (aep2.length !== 58) {
      return false;
    }
    const hexStr = ab2hexstring(base58.decode(aep2));
...
export function isAddress(address: string, verifyAddressVersion = -1): boolean {
  try {
    ...
    if (
      ...
      // Address might have come from a different network such as Atipicial Legacy.
      return false;
    }
    ...
    if (getAddressFromScriptHash(scriptHash, givenAddressVersion) !== address) {
      // address is not valid Atipicial address, could be btc, ltc etc.
      return false;
    }
    return true;
  } catch {
