...
import { sc, tx, u, rpc, wallet, CONST } from "@atipicial/atipicial-core";
import * as TestHelpers from "../../../../testHelpers";

let rpcClient: rpc.AtipicialServerRpcClient;
beforeAll(async () => {
  const url = await TestHelpers.getIntegrationEnvUrl();
  rpcClient = new rpc.AtipicialServerRpcClient(url);
}, 20000);
...
describe.skip("validateScript", () => {
  test("valid", async () => {
    const script = sc.createScript({
      scriptHash: CONST.NATIVE_CONTRACT_HASH.AtipicialCoin,
      operation: "symbol",
      args: [],
    });
    const transaction = new tx.Transaction({
...
describe("validateSystemFee", () => {
  ...
  describe("invalid", () => {
    ...
    test("script execution error in atipicialVM", async () => {
