...
- API
  ...
  Atipicial.api.setupVote(config);
  ```
...
- Smart Contract
  ...
  ```js
  import { sc } from "@atipicial/atipicial-js";
...
- API
  ...
  - Atipicialscan `getTransactionHistory` is updated to use a paginated endpoint. It
...
- RPC
  ...
  ```js
  import {rpc} from '@atipicial/atipicial-js
...
- API
  ...
  - Add attribute and remark for signing empty transactions. Now if you use
    ...
    transaction to be accepted by the ATC node without the need for any asset
    inputs.
...
- Fixes
  ...
  - Update `atipicialscan.getMaxClaimAmount` to use `get_unclaimed` endpoint
...
- Transaction
  ...
  ```js
  import { tx, settings } from "@atipicial/atipicial-js";
...
- Settings
  ...
  - There is now global settings that control how `atipicial-js` works. The first
    settings available are `httpsOnly` and `networks`.

    - `httpsOnly` is a boolean which will force atipicialDB and atipicialscan to return
    ...
    - `networks` is an object containing every network configuration available
      ...
      For now, these configurations only determine the atipicialDB/atipicialscan urls used
      for each network but they will serve more purposes in the future.

- RPC

  - Network class representing a ATC network. This can be created by importing a
    protocol file or just manually entering the parameters. This is the new way
    which we can link `atipicial-js` to a private net that comes with a atipicialscan or
    atipicialDB setup.
...
- Fixes
  ...
  - Fix atipicialDB and atipicialscan `getTransactionHistory`. **Do note that this can be
  ...
  - Typescript typings refactored and cleaned up. Introduces the object-like
    interfaces which are the atipicial-js classes exported as plain javascript
    objects.
...
- Fixes
  ...
  - atipicialDB to use `get_all_nodes` endpoint instead of `best_node` endpoint
  - Fix atipicialscan `getMaxClaimAmount`, `getBalance` and `getClaims` for invalid
    addresses
...
- API
  ...
  - Mark `api.aep5.doTransferToken` for deprecation.
...
- API
  ...
  - Expose the `loadBalance` method. This method allows the use of the dynamic
    API switch within `atipicial-js` for the external providers.
  - Implement the full set of APIs on atipicialscan. Atipicialscan is now complete and up to
    par with atipicialDB in terms of features.
...
```js
import { u } from "@atipicial/atipicial-js";
...
- Wallet
  ...
  - `Claims` is now a ES6 class. While it does not have any spceial methods for
    ...
    `atipicial-js`. This allows easy creation of `Claims` using the constructor by
  ...
  - `components` have been created for the minor sub-components found in
    ...
    components that are usable with `atipicial-js` methods. Refer to the wallet
    section for more information.
  - **BREAKING** Excess AEP2 functions are now fully deprecated. The list is
...
- Transaction
  ...
  - Update all methods to conform with the new Fixed8 classes. This also means
    that `atipicial-js` will not be compatible with just normal javascript objects
    anymore.

- API

  - **BREAKING** Update external API libraries (atipicialDB and atipicialscan) to return
  ...
  - A bug has been discovered in `getPrice`. While it works for ATC and GAS, it
    will not work for any of the AEP5 tokens. `getPrices` has been fixed for
