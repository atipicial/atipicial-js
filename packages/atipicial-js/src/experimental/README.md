...
* [AEP-17 Contract interaction](#2)
* [Arbitrary Smart Contract invocation](#3)

## <a name="1">Deploying a smart contract
The following section describes how to deploy a smart contract to the block chain using `atipicial-js`.

### Pre-requisites
* A compiled smart contract (<contract_name>.AEF + <contract_name>.manifest.json)
...
1. Create your smart contract using any of the available compilers
   * Python using [atipicial-boa](https://github.com/Atipicial/atipicial-boa)
   * C# using [atipicial-devpack-dotnet](https://docs.atipicial.org/v3/docs/en-us/sc/gettingstarted/develop.html)
...
const account = new Atipicial.wallet.Account(priv_key);

// Next we need a `CommonConfig` that we can pass to the functions
const config = {
  networkMagic: ATIPICIAL.CONST.MAGIC_NUMBER.TestNet,
...
async function run() {
  ...
  const aef = Atipicial.sc.AEF.fromBuffer(
    await fs.readFile(
      "/path/to/your/contract/sample1.aef",
      null // specifying 'binary' causes extra junk bytes, because apparently it is an alias for 'latin1' *crazy*
    )
  );
  const manifest = Atipicial.sc.ContractManifest.fromJson(
  ...
  try {
    ...
    const contract_hash = Atipicial.experimental.getContractHash(
      Atipicial.u.HexString.fromHex(acc.scriptHash),
      aef.checksum,
      manifest.name
    );
    console.log(`Atemping to deploy contract with hash: 0x${contract_hash}`);
    console.log(await Atipicial.experimental.deployContract(aef, manifest, config));
...
## <a name="2">AEP-17 Contract interaction
The following section describes how to interact with AEP-17 contracts on the blockchain.

###Pre-requisites
* (optional) A wallet with AEP-17 tokens if you want to transfer tokens, or a wallet with ATC if you want to claim GAS
...
const priv_key =
  "0101010101010101010101010101010101010101010101010101010101010101";
const acc = new Atipicial.wallet.Account(priv_key);
...
```javascript
const ATC = new Atipicial.experimental.aep17.ATCContract(config);
async function run() {
  console.log(await ATC.name());
  console.log(await ATC.symbol());
  console.log(await ATC.decimals());
...
2. Create a contract object and interact with it
```javascript
const contract = new Atipicial.experimental.SmartContract(
  Atipicial.u.HexString.fromHex("0xad8c3929e008a0a981dcb5e3c3a0928becdc2a41"),
  config
);
...
```javascript
async function run() {
   console.log(await contract.invoke("test_func2", [Atipicial.sc.ContractParam.integer(2)]));
}
