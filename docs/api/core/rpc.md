...
```js
import Atipicial, { rpc } from "@atipicial/atipicial-js";
const client = Atipicial.create.rpcClient(URL);
const alternative = new rpc.RPCClient(URL);
```

This module contains the classes and interfaces to interact with the RPC
endpoints supplied by the current C# atipicial implementation. It also contains the
...
The RPC Client acts as a model for a specific ATC Node. RPC Calls are methods
which external applications can interact with the ATC network easily without
sending a transaction.

It provides built-in RPC methods for easy calling. Previous queries can be
retrieved from the `history` property.

RPC methods mirror the API reference found in the official ATC documentation.
...
```js
// Creates a RPCClient that will talk to http://seed1.atipicial.com:10332
const client = Atipicial.create.rpcClient("http://seed1.atipicial.com:10332");
const alternative = new rpc.RPCClient("http://seed1.atipicial.com:10332");
...
let query = Atipicial.create.query({ method: "custommethod" });
...
```js
// Custom query
const query = Atipicial.create.query({ method: "newmethod", params: [arg1, arg2] });
const response = query.execute("http://mycustomatipicialode.com:10332");

// Simple query creation and execution
const response = rpc.Query.getBlock(1).execute("http://seed1.atipicial.com:10332");
```

### Network

The Network class is a configuration object that contains the information
required to connect to a blockchain. The default networks avaialble in `atipicial-js`
can be found in the global settings object. This class can be used to add
support for a private network to `atipicial-js`.

```js
const newNet = new rpc.Network({ name: "NewNet" });
Atipicial.add.network(newNet);

console.log(Atipicial.settings.networks["NewNet"]);
```
