...
```js
import Atipicial, { sc } from "@atipicial/atipicial-js";
const sb = Atipicial.create.scriptBuilder();
...
For example, in the AEP-17 token standard, we do not require an actual transaction
...
rpc.Query.invokeScript(sb.str)
  .execute("http://seed1.atipicial.com:10332")
  .then((res) => {
    console.log(res);
  });
```
...
```js
const props = {
  scriptHash: Atipicial.CONST.NATIVE_CONTRACT_HASH.AtipicialCoin,
  operation: "symbol",
  args: [],
};
// Returns a hexstring
const vmScript = Atipicial.create.script(props);
```
...
```js
const param1 = Atipicial.create.contractParam("String", "balanceOf");
