## API Report File for "@atipicial/atipicial-api"

> Do not edit this file. It is a report generated by [API Extractor](https://api-extractor.com/).

```ts

import { rpc } from '@atipicial/atipicial-core';
import RPCClient from '@atipicial/atipicial-core/lib/rpc/RPCClient';
import { sc } from '@atipicial/atipicial-core';
import { tx } from '@atipicial/atipicial-core';
import { u } from '@atipicial/atipicial-core';
import { wallet } from '@atipicial/atipicial-core';

// @public @deprecated
export function calculateNetworkFee(txn: tx.Transaction, feePerByte: number | u.BigInteger, executionFeeFactor: number | u.BigInteger): u.BigInteger;

// @public
export function getFeeInformation(client: rpc.AtipicialServerRpcClient): Promise<{
    feePerByte: u.BigInteger;
    executionFeeFactor: u.BigInteger;
}>;

// @public (undocumented)
export function getTokenBalances(address: string, contracts: (string | sc.Aep17Contract)[], client: rpc.AtipicialServerRpcClient): Promise<string[]>;

// @public (undocumented)
export function getTokenInfos(contracts: (string | sc.Aep17Contract)[], client: rpc.AtipicialServerRpcClient): Promise<TokenInfo[]>;

// @public (undocumented)
export interface Aep17TransferIntent {
    // (undocumented)
    contractHash: string;
    // (undocumented)
    decimalAmt?: number | string;
    // (undocumented)
    from: wallet.Account;
    // (undocumented)
    integerAmt?: number | string | u.BigInteger;
    // (undocumented)
    to: string;
}

// @public (undocumented)
export class NetworkFacade {
    claimGas(acct: wallet.Account, config: signingConfig): Promise<string>;
    // (undocumented)
    client: rpc.AtipicialServerRpcClient;
    // (undocumented)
    static fromConfig(config: NetworkFacadeConfig): Promise<NetworkFacade>;
    // Warning: (ae-forgotten-export) The symbol "Candidate" needs to be exported by the entry point index.d.ts
    getCandidates(): Promise<Candidate[]>;
    // (undocumented)
    getRpcNode(): rpc.AtipicialServerRpcClient;
    // (undocumented)
    initialize(): Promise<void>;
    // (undocumented)
    invoke(contractCall: sc.ContractCall): Promise<rpc.InvokeResult>;
    // (undocumented)
    magicNumber: number;
    sign(txn: tx.Transaction, config: signingConfig): Promise<tx.Transaction>;
    transferToken(intents: Aep17TransferIntent[], config: signingConfig): Promise<string>;
    validate(txn: tx.Transaction): Promise<ValidationResult>;
    // (undocumented)
    vote(acct: wallet.Account, candidatePublicKey: string, config: signingConfig): Promise<string>;
}

// @public (undocumented)
export interface NetworkFacadeConfig {
    // (undocumented)
    node: string | rpc.AtipicialServerRpcClient;
}

// @public (undocumented)
export interface signingConfig {
    // (undocumented)
    signingCallback: SigningFunction;
}

// @public (undocumented)
export type SigningFunction = (tx: tx.Transaction, details: {
    network: number;
    witnessIndex: number;
}) => Promise<string>;

// @public (undocumented)
export function signWithAccount(acct: wallet.Account): SigningFunction;

// @public (undocumented)
export function smartCalculateNetworkFee(txn: tx.Transaction, client: RPCClient): Promise<u.BigInteger>;

// @public (undocumented)
export interface TokenInfo {
    // (undocumented)
    decimals: number;
    // (undocumented)
    symbol: string;
    // (undocumented)
    totalSupply: string;
}

// @public (undocumented)
export class TransactionBuilder {
    addBasicSignatureField(account: wallet.Account): this;
    addContractCall(...contractCalls: sc.ContractCall[]): this;
    addEmptyWitness(account: wallet.Account): this;
    // (undocumented)
    addEmptyWitnesses(...accounts: wallet.Account[]): this;
    addGasClaim(account: wallet.Account): TransactionBuilder;
    addAep17Transfer(account: wallet.Account, destination: string, tokenScriptHash: string, amt: number | string | u.BigInteger): TransactionBuilder;
    // (undocumented)
    addScript(hexString: string): this;
    addSigners(...signers: tx.SignerLike[]): this;
    addVote(account: wallet.Account, candidatePublicKey: string): TransactionBuilder;
    // (undocumented)
    build(): tx.Transaction;
    // (undocumented)
    static newBuilder(): TransactionBuilder;
    setFeeAccount(account: wallet.Account): this;
    // (undocumented)
    setNetworkFee(fee: u.BigInteger): this;
    // (undocumented)
    setSystemFee(fee: u.BigInteger): this;
}

// @public
export class TransactionSigner {
    constructor(transaction: tx.Transaction);
    signWithAccount(...accounts: (wallet.Account | string)[]): void;
    signWithMultiSigAccount(multisigAccount: wallet.Account, ...witnesses: tx.Witness[]): void;
    signWithWitness(...witnesses: tx.Witness[]): void;
    transaction: tx.Transaction;
}

// @public
export class TransactionValidator {
    constructor(rpc: rpc.AtipicialServerRpcClient, transaction: tx.Transaction);
    rpcClient: rpc.AtipicialServerRpcClient;
    transaction: tx.Transaction;
    // (undocumented)
    static TX_LIFESPAN_SUGGESTION: number;
    // (undocumented)
    validate(attrs: ValidationAttributes, autoFix?: ValidationAttributes): Promise<ValidationResult>;
    validateNetworkFee(autoFix?: boolean): Promise<ValidationSuggestion<u.BigInteger>>;
    validateScript(): Promise<ValidationSuggestion<void>>;
    validateSystemFee(autoFix?: boolean): Promise<ValidationSuggestion<u.BigInteger>>;
    validateValidUntilBlock(autoFix?: boolean): Promise<ValidationSuggestion<number>>;
}

// @public (undocumented)
export enum ValidationAttributes {
    // (undocumented)
    All = 15,
    // (undocumented)
    NetworkFee = 4,
    // (undocumented)
    None = 0,
    // (undocumented)
    Script = 8,
    // (undocumented)
    SystemFee = 2,
    // (undocumented)
    ValidUntilBlock = 1
}

// @public (undocumented)
export interface ValidationResult {
    // (undocumented)
    result: {
        validUntilBlock?: ValidationSuggestion<number>;
        script?: ValidationSuggestion<void>;
        systemFee?: ValidationSuggestion<u.BigInteger>;
        networkFee?: ValidationSuggestion<u.BigInteger>;
    };
    valid: boolean;
}

// @public (undocumented)
export type ValidationSuggestion<T> = {
    valid: boolean;
    fixed: boolean;
    prev?: T;
    suggestion?: T;
    message?: string;
};

// (No @packageDocumentation comment for this package)

```
