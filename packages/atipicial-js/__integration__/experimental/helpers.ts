...
beforeAll(async () => {
  ...
  aef = sc.AEF.fromBuffer(
    await fs.readFile(
      path.resolve(__dirname, "./contract3.aef"),
...
describe("contract", () => {
  test("deploy & invoke", async () => {
    ...
    const txid = await experimental.deployContract(aef, manifest, config);
  ...
  test("deploy duplicate contract", async () => {
    ...
    //
    try {
      await experimental.deployContract(aef, manifest, config);
    } catch (e) {
  ...
  test("deploy duplicate contract - with fee override", async () => {
    ...
    // If one does not override the systemfee then this call will fail while trying to obtain the system fee through RPC
    const txid = await experimental.deployContract(aef, manifest, config2);
    expect(txid).toBeDefined();
