...
```sh
npm i @atipicial/atipicial-node-ledger @atipicial-atipicial-core
```
...
```js
const bipString = AtipicialJs.ledger.BIP44(1);
```
...
const paths = await atipicialJs.ledger.getDevicePaths(LedgerLibrary);
...
```js
const publicKey = await atipicialJs.ledger.getPublicKey(ledgerInstance, bipString);
```
...
```js
const signature = await atipicialJs.ledger.getSignature(
    ledgerInstance,
    tx,
    bipString
);
