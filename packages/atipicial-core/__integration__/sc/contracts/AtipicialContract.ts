...
import { AtipicialContract } from "../../../src/sc/contracts/AtipicialContract";
...
describe("AtipicialContract", () => {
  test("getCandidates", async () => {
    const call = AtipicialContract.INSTANCE.getCandidates();
  ...
  test("getRegisterPrice", async () => {
    const call = AtipicialContract.INSTANCE.getRegisterPrice();
    const response = await rpcClient.invokeFunction(
      call.scriptHash,
      call.operation,
      call.args,
  ...
  test("registerCandidate", async () => {
    const call = AtipicialContract.INSTANCE.registerCandidate(
  ...
  test("vote", async () => {
    const call = AtipicialContract.INSTANCE.vote(
