...
- Fixes
  ...
  - clean atipicialDB input numbers
...
- API
  ...
  - Update `doMintToken` to include extra information required for future
    ...
    - This is in preparation of the upcoming changes for minting AEP5 tokens.

  - Implement the API switch.

    - This internal switch allows control over priority of API server.
    - Set to 0 for atipicialscan priority, 1 for atipicialDB priority. Setting it in the
    ...
    api.setApiSwitch(0);
    api.sendAsset(config); // sendAsset, claimGas and doInvoke will default to use atipicialscan first
    api.setApiSwitch(1);
    api.doInvoke(config); // This call will default to use atipicialDB first
...
- Fixes
  ...
  - Fix AEP5 tokens to parse by `decimals` field.
...
- Wallet
  ...
  - Implementation of AEP-6 standard as `wallet.Wallet`
  - Move AEP2 constants to CONST
  ...
  const w1 = Atipicial.create.wallet();
...
- API
  ...
  - Add `api.getToken` which is a combination of `api.getTokenInfo` and
    ...
    call. This is exposed semantically as `Atipicial.get.token`.
...
```js
import { rpc, u } from "@atipicial/atipicial-js";
...
- Balance as an ES6 class.
  ...
  - `verifyAssets` to validate unspent coins against a given ATC node. Used to
  ...
  balance.assetSymbols = ['ATC', 'GAS']
  // Lookup assets using their symbols
  balance.assets = {
    ATC: {balance: 1, unspent: [ Object ], spent: [], unconfirmed: []}
    GAS: {balance: 25.1, unspent: [ Object ], spent: [], unconfirmed: []}
  }
  ```

- Added `doTransferToken` to `api/aep5`
...
- Package exports semantic style
  ...
  - Modules are individually available as named exports. If you just need
    account methods, `import { wallet } from '@atipicial/atipicial-js'`

- Constants and util methods are now exported as

  ```js
  import Atipicial from "@atipicial/atipicial-js";
  Atipicial.CONST.DEFAULT_RPC;
  Atipicial.u.reverseHex;

  import { CONST, u } from "@atipicial/atipicial-js";
  CONST.DEFAULT_RPC;
  u.reverseHex;
  ```
...
- RPC
  ...
  - RPCClient class models a ATC Node. Instantiate with
    `Atipicial.create.rpcClient(url)`. Comes with built-in methods for RPC calls.
  - Query class models a RPC call. Instantiate with `Atipicial.create.query()`. Comes
    with built-in methods for RPC calls.

- API

  - atipicial-wallet-db API is shifted to `api` folder.
  - Added coinmarketcap query support for easy price queries.
  - Token query (AEP5) is shifted here.
  - Atipicialscan support added.
