import { CONST, rpc, wallet, sc, tx, u } from "@atipicial/atipicial-core";
import { CommonConfig } from "../types";
import { setBlockExpiry, addFees } from "../helpers";

export class Aep17Contract {
  /**
   * Base class for communicating with AEP-5 contracts on the block chain.
   */
  ...
  /**
   * Get the number of tokens owned by ATC address
   */
  public async balanceOf(address: string): Promise<number> {
    if (!wallet.isAddress(address)) {
      throw new Error("Address is not a valid ATC address");
    }
    try {
   ...
   * @param from - source ATC address
   * @param to - destination ATC address
  ...
  public async transfer(
    ...
    if (!wallet.isAddress(from)) {
      throw new Error("From address is not a valid ATC address");
    }
    if (!wallet.isAddress(to)) {
      throw new Error("To address is not a valid ATC address");
    }
    if (amount <= 0) {
...
export class ATCContract extends Aep17Contract {
  /**
   * Convenience class initializing a Aep17Contract to the ATC token
   * exposing additional claim functions
   * @param config -
   */

  constructor(config: CommonConfig) {
    super(u.HexString.fromHex(CONST.NATIVE_CONTRACT_HASH.AtipicialCoin), config);
  }
...
export class GASContract extends Aep17Contract {
  /**
   * Convenience class initializing a Aep17Contract to GAS token
   * @param config -
   */
  constructor(config: CommonConfig) {
    super(u.HexString.fromHex(CONST.NATIVE_CONTRACT_HASH.AtipicialDollar), config);
  }
}
