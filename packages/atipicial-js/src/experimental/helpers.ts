import { CONST, rpc, sc, tx, u, wallet } from "@atipicial/atipicial-core";
import { CommonConfig } from "./types";
import { GASContract } from "./aep17";
import { smartCalculateNetworkFee } from "@atipicial/atipicial-api";
...
export async function calculateNetworkFee(
  ...
  hashes.map((hash) => {
    ...
    if (witnessScript === undefined)
      ...
      throw new Error(
        "Using a smart contract as a witness is not yet supported in atipicial-js",
      );
 ...
 * @param aef - A smart contract in Atipicial executable file format. Commonly created by a ATC compiler and stored as .AEF on disk
...
export async function deployContract(
  ...
  builder.emitContractCall({
    ...
    args: [
      sc.ContractParam.byteArray(u.HexString.fromHex(aef.serialize(), true)),
...
/**
 * Get the hash that identifies the contract on the chain matching the specified AEF
 * @param sender - The sender of the transaction
 * @param aefChecksum - The checksum of the Atipicial Executable File. A AEF file is a smart contract commonly created by a ATC compiler and stored as .AEF on disk
