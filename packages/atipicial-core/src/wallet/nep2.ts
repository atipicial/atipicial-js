/**
 * AEP2 - Private Key Encryption based on AES.
...
async function createAep2Key(
  ...
  const assembled =
    AEP2_HEADER + AEP2_FLAG + addressHash + encrypted.ciphertext.toString();
...
function getAtipicial2AddressFromPrivateKey(privateKey: string): string {
  ...
  const shaChecksum = hash256(ATC2_ADDRESS_VERSION + scriptHash).substr(0, 8);
  return base58.encode(
    Buffer.from(ATC2_ADDRESS_VERSION + scriptHash + shaChecksum, "hex"),
  );
}

/**
 * Encrypts a WIF key using a given keyphrase under AEP-2 Standard.
 ...
 * @param scryptParams - optional parameters for Scrypt. Defaults to AEP2 specified parameters.
...
export function encrypt(
  ...
  return createAep2Key(
    AEP2_HEADER + AEP2_FLAG,
    privateKey,
    keyphrase,
    address,
    scryptParams,
...
async function decipherAep2Key(
  encryptedKey: string,
  keyphrase: string,
...
/**
 * Decrypts an encrypted key using a given keyphrase under AEP-2 Standard.
 ...
 * @param scryptParams - parameters for Scrypt. Defaults to AEP2 specified parameters.
...
export async function decrypt(
  ...
  const privateKey = await decipherAep2Key(
    encryptedKey,
    keyphrase,
...
export async function decryptAtipicial2(
  encryptedKey: string,
  keyphrase: string,
  scryptParams: ScryptParams = DEFAULT_SCRYPT,
): Promise<string> {
  const privateKey = await decipherAep2Key(
    encryptedKey,
    keyphrase,
    getAtipicial2AddressFromPrivateKey,
    scryptParams,
  );
  return getWIFFromPrivateKey(privateKey);
}
