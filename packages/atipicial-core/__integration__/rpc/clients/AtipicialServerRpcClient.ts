...
// ATC contract hash. Should be same across TestNet or LocalNet.
const contractHash = CONST.NATIVE_CONTRACT_HASH.AtipicialCoin;
let txid: string;
...
describe("AtipicialServerRpcClient", () => {
  ...
  test("getNativeContracts", async () => {
    ...
    expect(contractNames).toContain("AtipicialCoin");
    expect(contractNames).toContain("AtipicialDollar");
  ...
  test("getStorage", async () => {
    ...
    // This storage is totalSupply of ATC. Should be safe and static for usage.
    expect(result).toBe("AOH1BQ==");
  });

  describe.only("findStorage", () => {
    test("find ATCs totalSupply storage", async () => {
      ...
      // This find will only get the totalSupply of ATC. Should be safe and static for usage.
    ...
    test("find ATCs balance storage", async () => {
      // This find will only get a list of accounts on ATCs storage. Will only be used to check the truncated and next values.
      ...
      // There isn't 50 or more accounts with ATC on this blockchain instance.
  ...
  describe("Invocation methods", () => {
    ...
    test("invokeScript", async () => {
      ...
      expect(result.stack[0].value).toEqual(
        u.HexString.fromAscii("ATC").toBase64(),
      );
    });
