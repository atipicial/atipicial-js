...
```js
import Atipicial, { wallet, CONST } from "@atipicial/atipicial-js";
const account = Atipicial.create.account(privateKey);
const alternative = new wallet.Account(privateKey);

Atipicial.is.address(string, CONST.DEFAULT_ADDRESS_VERSION);
...
```js
const a = Atipicial.create.Account("NNtxeX9UhKfHySqPQ29hQnZe22k8LwcFk1");
...
The `Wallet` class implements the AEP-6 convention which is a standard way set
by ATC council on how to export keys in a JSON file. By doing this, we can move
...
```js
import Atipicial, { wallet } from "atipicial/atipicial-js";
...
const w1 = Atipicial.create.wallet({ name: "myWallet" });
...
```js
const privateKey = Atipicial.create.privateKey();
const publicKey = Atipicial.get.publicKeyFromPrivateKey(publicKey);
const scriptHash = Atipicial.get.scriptHashFromPublicKey(publicKey);
const address = Atipicial.get.addressFromScriptHash(scriptHash);
...
These are methods used to convert JS objects into their respective `atipicial-js`
implementation.
...
The AEP2 standard describes the procedure to encrypt or decrypt a private key.
...
```js
const privateKey = Atipicial.create.privateKey();
const WIF = Atipicial.get.WIFFromPrivateKey(privateKey);
const aep2Key = await Atipicial.encrypt(privateKey, "myPassword");
const decryptedKey = await Atipicial.decrypt(aep2Key, "myPassword");
WIF === decryptedKey; // true

const privateKey = wallet.generatePrivateKey();
const WIF = new wallet.Account(privateKey).WIF;
const aep2Key = await wallet.encrypt(WIF, "myPassword");
const decryptedKey = await wallet.decrypt(aep2Key, "myPassword");
WIF === decryptedKey; // true
```
...
```js
Atipicial.is.address(addrStr);
Atipicial.is.privateKey(keyStr);
Atipicial.is.encryptedKey(encryptedStr);
Atipicial.is.publicKey(publicKeyStr);
Atipicial.is.wif(wifStr);
Atipicial.is.scriptHash(scriptHashStr);

wallet.isAddress(addrStr);
wallet.isPrivateKey(keyStr);
wallet.isAEP2(keyStr);
